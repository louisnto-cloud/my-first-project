...
# ============================================================ 5. Scenarios
sc=wb.create_sheet("Scenarios"); sc.sheet_properties.tabColor="7C7BFF"; sc.sheet_view.showGridLines=False
for col,w in {"A":2.5,"B":26,"C":15,"D":15,"E":15}.items(): sc.column_dimensions[col].width=w
title(sc,"Scenario planning — Base / Bull / Bear",5,
      "Bull and Bear scale your Base assumptions. Multipliers are editable; metrics derive from the channel totals.")
# multiplier block
sc.cell(row=4,column=2,value="Multiplier").font=font(10,True,"FFFFFF");
for i,nm in enumerate(["Base","Bull","Bear"]):
    c=sc.cell(row=4,column=3+i,value=nm); sty(c,f=font(10,True,"FFFFFF"),fl=fill(NAVY),al="center",bd=True)
sc.cell(row=4,column=2).fill=fill(NAVY); sc.cell(row=4,column=2).border=BORDER; sc.cell(row=4,column=2).alignment=Alignment(horizontal="left")
mults=[("Volume ×",[1,1.40,0.75]),("Price ×",[1,1.05,0.97]),("COGS ×",[1,0.90,1.10]),
       ("Trade ×",[1,0.85,1.15]),("Ad / CAC ×",[1,0.85,1.20])]
MR0=5
for k,(nm,vals) in enumerate(mults):
    rr=MR0+k; lbl(sc,f"B{rr}",nm)
    for i,v in enumerate(vals): inp(sc,f"{get_column_letter(3+i)}{rr}",v,"0.00")
MV,MP,MC,MT,MA=[MR0+x for x in range(5)]
# derived metrics rows
section(sc,11,"Resulting P&L",5)
hd=["Metric","Base","Bull","Bear"]
for i,h in enumerate(hd):
    c=sc.cell(row=12,column=2+i,value=h); sty(c,f=font(10,True,"FFFFFF"),fl=fill(NAVY),al="left" if i==0 else "right",bd=True)
Lt=f"Channels!$L${TR}"; Mt=f"Channels!$M${TR}"; Nt=f"Channels!$N${TR}"; Pt=f"Channels!$P${TR}"
Rt=f"Channels!$R${TR}"; St=f"Channels!$S${TR}"; Ttt=f"Channels!$T${TR}"; Ut=f"Channels!$U${TR}"
def col(i): return get_column_letter(3+i)
def net_s(i):  # mp*mv*(L - mt*M - N)
    c=col(i); return f"({c}{MP}*{c}{MV}*({Lt}-{c}{MT}*{Mt}-{Nt}))"
def gp_s(i):
    c=col(i); return f"({net_s(i)}-{c}{MC}*{c}{MV}*{Pt})"
def cm_s(i):
    c=col(i); return f"({gp_s(i)}-{c}{MP}*{c}{MV}*{Rt}-{c}{MA}*{c}{MP}*{c}{MV}*{St}-{c}{MV}*{Ttt}-{c}{MV}*{Ut})"
def ebitda_s(i):
    c=col(i); return f"({cm_s(i)}-{MK}*{net_s(i)}-{FX})"
metrics=[
 ("Annual cases",   lambda i:f"='P&L'!$C$5*{col(i)}{MV}", NUM),
 ("Net sales",      lambda i:f"={net_s(i)}", MONEY),
 ("Gross margin %", lambda i:f"={gp_s(i)}/{net_s(i)}", PCT),
 ("Contribution $", lambda i:f"={cm_s(i)}", MONEY),
 ("Contribution %", lambda i:f"={cm_s(i)}/{net_s(i)}", PCT),
 ("EBITDA $",       lambda i:f"={ebitda_s(i)}", MONEY),
 ("EBITDA %",       lambda i:f"={ebitda_s(i)}/{net_s(i)}", PCT),
]
rr=13
for name,fn,nf in metrics:
    lbl(sc,f"B{rr}",name,b=(name.startswith("EBITDA")))
    for i in range(3):
        c=sc.cell(row=rr,column=3+i,value=fn(i)); sty(c,f=font(11,name.startswith("EBITDA")),nf=nf,al="right",bd=True)
        if name=="EBITDA $":
            sc.conditional_formatting.add(c.coordinate,CellIsRule(operator="lessThan",formula=["0"],fill=fill(RED)))
            sc.conditional_formatting.add(c.coordinate,CellIsRule(operator="greaterThanOrEqual",formula=["0"],fill=fill(GRN)))
    rr+=1
...
G=[("","" ),
 ...
 ("Scenarios    – Base / Bull / Bear via editable multipliers on the channel totals.","" ),
 ("",""),
