"""Build Organika RTD weekly forecast workbook (parametric v2).

Same model as v1, but every input is now editable in one place and
propagates through:
  - Doors: 12 monthly input cols per row drive 52 formula-based weekly cols
  - Rep samples: 12 monthly inputs per province drive 52 weekly outputs
  - Marketing buffer: single "cases per week" rate per item fills all 52 weeks
  - Events: total cases + start week + end week distribute evenly across window
  - VLOOKUP ranges extended so new channels/SKUs work without editing lookups
"""
...
# 52 weeks starting Tue Sep 1 2026.
...
# ---- Standard Forecast/Marketing/Production weekly columns (no monthly input strip) ----
# These sheets use cols E..BD for W1..W52.
FW_FIRST_COL = 5
FW_LAST_COL = FW_FIRST_COL + N_WEEKS - 1  # 56 = BD

def fw_week_col(i):
    return get_column_letter(FW_FIRST_COL + i - 1)

# ---- Doors layout (monthly inputs strip then weekly outputs strip) ----
# Cols A-D = Brand, Province, Channel, FY total
# Cols E-P (5-16) = 12 monthly inputs
# Cols Q-BP (17-68) = 52 weekly formula outputs
DOORS_MONTHLY_FIRST_COL = 5
DOORS_MONTHLY_LAST_COL = 16  # P
DOORS_WEEKLY_FIRST_COL = 17  # Q
DOORS_WEEKLY_LAST_COL = DOORS_WEEKLY_FIRST_COL + N_WEEKS - 1  # 68 = BP

MONTH_INPUT_COL = {
    m: get_column_letter(DOORS_MONTHLY_FIRST_COL + i) for i, m in enumerate(MONTH_ORDER)
}

def doors_week_col(i):
    return get_column_letter(DOORS_WEEKLY_FIRST_COL + i - 1)

# ---- Rep samples layout (same shape as Doors) ----
REP_MONTHLY_FIRST_COL = 2  # B
REP_MONTHLY_LAST_COL = REP_MONTHLY_FIRST_COL + 11  # M
REP_WEEKLY_FIRST_COL = 14  # N
REP_WEEKLY_LAST_COL = REP_WEEKLY_FIRST_COL + N_WEEKS - 1  # 65 = BM

REP_MONTH_INPUT_COL = {
    m: get_column_letter(REP_MONTHLY_FIRST_COL + i) for i, m in enumerate(MONTH_ORDER)
}
...
MONTHLY_FILL = PatternFill("solid", fgColor="FFF8E5")  # subtle highlight for monthly input strip
...
def build_readme(ws):
    ws.title = "README"
    ws["A1"] = "Organika RTD National Forecast - FY26 to FY27 (Weekly, 9-channel, parametric build)"
    ...
    rows = [
        ...
        ("Cream fill", "Monthly input strip. Edit one cell, all 4-5 weeks in that month update."),
        ("", ""),
        ("How to edit", ""),
        ("Doors", "Edit the 12 monthly input columns (cream fill). Weekly columns are auto-filled."),
        ("Rep samples", "Edit the 12 monthly input columns. Weekly columns are auto-filled."),
        ("Marketing buffer", "Edit one 'Cases per week' cell per line item; all 52 weeks follow."),
        ("Events", "Edit Total cases, Start week, End week. Cases spread evenly across that window."),
        ("Account sampling", "Edit one 'Cases per door per week' rate per channel; all weeks recompute."),
        ("", ""),
        ...
        ("Doors", "Province by Channel by Brand active door counts. Monthly inputs -> weekly outputs."),
        ...
        ("", ""),
        ("New-channel default inputs are placeholders flagged yellow. Sign off with Aaron, Teresa, Rijo before locking.", ""),
    ]
    for i, (a, b) in enumerate(rows, start=3):
        ws.cell(row=i, column=1, value=a).font = HEADER_FONT if a and not b == "" and i in (10, 17, 24, 28, 32, 38) else Font()
...
def build_assumptions(ws):
    ...
    ws["A13"] = "Week"; ws["B13"] = "Start date"; ws["C13"] = "Month"
...
def build_class_mix(ws):
    ...
    write_headers(ws, 5, ["Channel", "Class A %", "Class B %", "Class C %", "Sum check"])
    ...
    ws["A17"] = "Class definitions"; ws["A17"].font = SECTION_FONT
    defs = [
...
def build_velocity(ws):
    ...
    write_headers(ws, 5, ["SKU", "Channel", "Vel A", "Vel B", "Vel C",
    ...
    # Use generous lookup range so adding channels doesn't break things.
...
def build_pricing(ws):
    ...
    write_headers(ws, 5, ["SKU", "Net case price", "Landed case cost", "Gross profit", "GP %", "Notes"])
...
def build_doors(ws):
    """Build Doors with 12 monthly input cols then 52 formula-driven weekly cols."""
    ws.title = "Doors"
    ws["A1"] = "Active doors by brand, province, and channel"
    ws["A1"].font = TITLE_FONT
    ws["A3"] = ("Main planning canvas. Edit the 12 monthly input columns (cream fill); "
                "the 52 weekly columns auto-fill. New-channel rows are yellow placeholders.")

    # Headers
    headers_static = ["Brand", "Province", "Channel", "FY total"]
    for col_idx, val in enumerate(headers_static, start=1):
        c = ws.cell(row=5, column=col_idx, value=val)
        c.font = HEADER_FONT; c.fill = HEADER_FILL
    # Monthly input headers
    for j, m in enumerate(MONTH_ORDER):
        c = ws.cell(row=5, column=DOORS_MONTHLY_FIRST_COL + j, value=m)
        c.font = HEADER_FONT; c.fill = HEADER_FILL
        c.alignment = Alignment(horizontal="center")
    # Section banner above month inputs
    banner = ws.cell(row=4, column=DOORS_MONTHLY_FIRST_COL, value="Monthly inputs (edit these)")
    ...
    for j, lbl in enumerate(WEEK_LABELS):
        ...
        c.font = HEADER_FONT; c.fill = HEADER_FILL
        c.alignment = Alignment(horizontal="center")
    banner2 = ws.cell(row=4, column=DOORS_WEEKLY_FIRST_COL, value="Weekly outputs (formulas - do not edit)")
    ...
    # Add month reference under weekly headers (row 6 below where data starts... actually put above row 5? no, row 5 is headers)
    # Skip subheader row, data starts at 7 to leave room for month reference row 6
    # Actually keep data starting at row 6 to keep simple; reference row not strictly needed.
    ...
    # --- MUV existing channels (from original) ---
    ...
    for ch, prov_map in muv_new.items():
    ...
    for brand in ("MUV", "LCA"):
        for prov in PROVINCES:
            for ch in CHANNELS:
                ...
                # FY total = sum of weekly columns
                ...
                for j, m in enumerate(MONTH_ORDER):
                    ...
                    if ch in NEW_CHANNELS:
                        cell.fill = YELLOW_FILL
                # Weekly formulas pulling from the month col
    ...
    first_wk = doors_week_col(1); last_wk = doors_week_col(N_WEEKS)
    ws.cell(row=total_row, column=4, value=f"=SUM(D6:D{total_row - 1})").font = HEADER_FONT
    # Subtotal monthly inputs as well
    for j, m in enumerate(MONTH_ORDER):
        ...
        c.font = HEADER_FONT; c.fill = SUBTOTAL_FILL
    for wi in range(1, N_WEEKS + 1):
...
def build_marketing(ws):
    ...
    # --- Section 1: Account sampling (already parametric) ---
    sec1_header = 13
    ws.cell(row=sec1_header, column=1,
            value="Section 1: Account sampling (1 rate per channel drives 52 weeks)").font = SECTION_FONT
    ...
    for i, ch in enumerate(CHANNELS):
        ...
        # Each week: rate * sum of doors in that channel for that week (use DOORS_WEEKLY columns)
        for wi in range(1, N_WEEKS + 1):
            col = FW_FIRST_COL + wi - 1
            dwc = doors_week_col(wi)
            ws.cell(row=r, column=col,
                    value=(f"=$B{r}*SUMIFS(Doors!{dwc}:{dwc},"
                           f"Doors!$C:$C,$A{r})")).font = LINK_FONT
    sec1_last = sec1_first + len(CHANNELS) - 1
    sec1_total = sec1_last + 1
    ws.cell(row=sec1_total, column=1, value="Account sampling total").font = HEADER_FONT
    for wi in range(1, N_WEEKS + 1):
        col = FW_FIRST_COL + wi - 1
        colL = get_column_letter(col)
    ...
    # --- Section 2: Events (parametric: total + start week + end week) ---
    sec2_header = sec1_total + 2
    ws.cell(row=sec2_header, column=1,
            value="Section 2: Events and activations (edit Total / Start / End)").font = SECTION_FONT
    sub = sec2_header + 1
    ...
    for i, (lbl, total, start, end) in enumerate(events):
        ...
        # Each weekly cell distributes total / (end - start + 1) if in window, else 0
        for wi in range(1, N_WEEKS + 1):
            col = FW_FIRST_COL + wi - 1
            ws.cell(row=r, column=col,
    ...
    ws.cell(row=sec2_total, column=1, value="Events total").font = HEADER_FONT
    for wi in range(1, N_WEEKS + 1):
        col = FW_FIRST_COL + wi - 1
        colL = get_column_letter(col)
    ...
    # --- Section 3: Rep samples (monthly inputs per province drive 52 weeks) ---
    sec3_header = sec2_total + 2
    ws.cell(row=sec3_header, column=1,
            value="Section 3: Rep samples (edit monthly rep counts; weekly auto-fills)").font = SECTION_FONT
    ...
    ws.cell(row=total_reps_row, column=1, value="Total reps active").font = HEADER_FONT
    ws.cell(row=sample_row, column=1, value="Rep sample cases total").font = HEADER_FONT
    for wi in range(1, N_WEEKS + 1):
        col = REP_WEEKLY_FIRST_COL + wi - 1
        colL = get_column_letter(col)
        ws.cell(row=total_reps_row, column=col,
                value=(f"=SUM({colL}{sec3_first}:{colL}{sec3_first + len(PROVINCES) - 1})")).font = HEADER_FONT
        c = ws.cell(row=sample_row, column=col,
                    value=f"=$B${rate_row}*{colL}{total_reps_row}")
        c.font = HEADER_FONT; c.fill = SUBTOTAL_FILL

    # Also link sample_row total into the standard FW weekly columns (so the
    # summary at top can pick it up using the standard E..BD layout).
    # We project each rep-weekly column at REP_WEEKLY_FIRST_COL + wi-1 into the
    # standard FW column at FW_FIRST_COL + wi-1 of a hidden link row.
    rep_link_row = sample_row + 1
    ws.cell(row=rep_link_row, column=1,
            value="(Rep samples link to standard week columns)").font = Font(italic=True, color="808080")
    for wi in range(1, N_WEEKS + 1):
        src = get_column_letter(REP_WEEKLY_FIRST_COL + wi - 1)
        dst_col = FW_FIRST_COL + wi - 1
        ws.cell(row=rep_link_row, column=dst_col,
                value=f"={src}{sample_row}").font = LINK_FONT

    # --- Section 4: Buffer (single rate per item drives 52 weeks) ---
    sec4_header = rep_link_row + 2
    ws.cell(row=sec4_header, column=1,
            value="Section 4: Buffer (edit one 'Cases per week' per item)").font = SECTION_FONT
    sub4 = sec4_header + 1
    ...
    ws.cell(row=sec4_total, column=1, value="Buffer total").font = HEADER_FONT
    for wi in range(1, N_WEEKS + 1):
        col = FW_FIRST_COL + wi - 1
        colL = get_column_letter(col)
...
def build_forecast_weekly(ws):
    ...
    # Generous lookup ranges
    vel_range = "Velocity!$A$6:$I$200"
        ...
        for prov in PROVINCES:
            for ch in CHANNELS:
                ...
                for wi in range(1, N_WEEKS + 1):
                    ...
                    dwc = doors_week_col(wi)
                    formula = (
                        f"=SUMIFS(Doors!{dwc}:{dwc},Doors!$A:$A,$D{row},"
...
def build_production_plan(ws, forecast_total_row, marketing_summary):
    ...
    ws.cell(row=total_sales_row, column=fy_col,
            value=(f"=SUM({fw_week_col(1)}{total_sales_row}:"
                   f"{fw_week_col(N_WEEKS)}{total_sales_row})")).font = HEADER_FONT
    ...
    mk_rows = [
        ("Account sampling",        marketing_summary["summary_acct"]),
    ...
    ws.cell(row=mk_total_row, column=fy_col,
            value=(f"=SUM({fw_week_col(1)}{mk_total_row}:"
                   f"{fw_week_col(N_WEEKS)}{mk_total_row})")).font = HEADER_FONT
    ...
    for wi in range(1, N_WEEKS + 1):
        ...
        ws.cell(row=total_prod_row, column=col,
                value=f"={colL}{total_sales_row}+{colL}{mk_total_row}").font = HEADER_FONT
    ws.cell(row=total_prod_row, column=fy_col,
            value=(f"=SUM({fw_week_col(1)}{total_prod_row}:"
                   f"{fw_week_col(N_WEEKS)}{total_prod_row})")).font = HEADER_FONT
    ...
    ws.cell(row=buf_row, column=fy_col,
            value=(f"=SUM({fw_week_col(1)}{buf_row}:"
                   f"{fw_week_col(N_WEEKS)}{buf_row})")).font = HEADER_FONT
...
def build_revenue(ws):
    ...
    ws["A5"] = "Revenue (CAD)"; ws["A5"].font = SECTION_FONT
    fy_col = FW_FIRST_COL + N_WEEKS

    # Generous Pricing lookup range
...
def build_dashboard(ws, prod_refs, rev_refs, doors_total_row, forecast_total_row):
    ws.title = "Dashboard"
    ws["A1"] = "Organika RTD national forecast dashboard - FY26 to FY27 (weekly, parametric)"
    ws["A1"].font = TITLE_FONT
    ...
    ws["A5"] = "Headline metrics"; ws["A5"].font = SECTION_FONT
    ...
    headline = [
        ...
        ("Active doors at year end (W52)",     f"=Doors!{doors_week_col(N_WEEKS)}{doors_total_row}", "Doors national total, W52"),
    ...
    # Active doors by channel (use Doors monthly input column for the month directly)
    door_base = prov_total_r + 3
    ws.cell(row=door_base, column=1, value="Active doors by channel (monthly input)").font = SECTION_FONT
