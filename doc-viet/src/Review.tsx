...
import { todayISO, XP, type DVProgress } from './engine';
import { viCloseMatch, viEquals } from './vi';
import { useTTS } from './useTTS';
import Flashcards from './Flashcards';
import { Confetti, NavButton, StepCard, ToneHelperRow, XpChip } from './shared';
...
function learnedVocab(progress: DVProgress): { word: string; meaning: string }[] {
...
function buildSession(progress: DVProgress, size = 8): ReviewQ[] {
...
export default function Review({ progress, apply }: {
  ...
  const choose = (opt: string) => {
    if (!q || picked) return;
    setPicked(opt);
    const right = viEquals(opt, q.word);
    ...
    return (
      <StepCard>
        <div className="py-8 text-center">
          ...
          <NavButton label="🃏 Flashcards — First Words →" onClick={() => setShowFlash(true)} color="bg-emerald-600" />
        </div>
      </StepCard>
    );
  }
    ...
    return (
      <StepCard>
        ...
        <div className="py-6 text-center">
          ...
          <NavButton label="Review Again 🔁" onClick={start} color="bg-red-600" />
        </div>
      </StepCard>
    );
  }
  ...
  // ── In-session question ──
  if (session && q) {
    const typedWrong = picked !== null && !viEquals(picked, q.word);
    const typedClose = typedWrong && viCloseMatch(picked!, q.word);
    return (
      <StepCard>
        ...
        <div className="h-1.5 w-full overflow-hidden rounded-full bg-gray-100">
          <div className="h-1.5 rounded-full bg-red-500 transition-all" style={{ width: `${((qIndex + (picked ? 1 : 0)) / session.length) * 100}%` }} />
        </div>
        <div className="py-3 text-center">
          <div className="text-xs font-bold uppercase tracking-wide text-red-500">
            {q.mode === 'type' ? '⭐ Recall — type the Vietnamese word that means…' : 'Which word means…'}
          </div>
          <p className="mt-2 text-lg font-black text-gray-800">"{q.meaning}"</p>
          <button onClick={() => tts.speak(q.word)} className="mt-1 text-xs text-gray-400 hover:text-gray-600">🔊 hear the word</button>
        </div>
        ...
        {q.mode === 'choice' ? (
          ...
          <div className="space-y-2">
            <input
              ...
              placeholder={`Type the word — diacritics required! (starts with "${q.word[0]}")`}
              autoCapitalize="none" autoCorrect="off" spellCheck={false}
              className={`w-full rounded-xl border-2 px-3 py-2.5 text-sm font-bold focus:outline-none ${
                picked
                  ? viEquals(picked, q.word) ? 'border-green-400 bg-green-50' : 'border-red-300 bg-red-50'
                  : 'border-gray-200 focus:border-red-400'}`}
            />
            {!picked && <ToneHelperRow value={typed} onChange={setTyped} />}
            {!picked ? (
              <button onClick={() => typed.trim() && choose(typed)} disabled={!typed.trim()}
                className="w-full rounded-xl bg-red-600 py-2.5 text-sm font-black text-white disabled:opacity-40">
                Check
              </button>
            ) : typedWrong && (
              <p className="text-center text-sm font-bold text-red-600">
                The word was: {q.word}
                {typedClose && <span className="mt-1 block text-xs font-bold text-amber-700">🎵 So close! Only your tone marks were off — check the hats and accents.</span>}
              </p>
            )}
          </div>
        )}
        {picked && (
          <NavButton label={qIndex + 1 >= session.length ? 'Finish Session →' : 'Next Question →'} onClick={next} color="bg-red-600" />
        )}
      </StepCard>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl bg-gradient-to-br from-red-500 to-red-700 p-5 text-white shadow-lg">
      ...
      <StepCard>
        ...
        <NavButton label="Start Review Session (8 questions) →" onClick={start} color="bg-red-600" />
