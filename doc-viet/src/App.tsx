...
export default function App() {
  const [progress, setProgress] = useState<DVProgress>(loadProgress);
   ...
   * side effects (localStorage) that must not double-fire under StrictMode.
   */
  const apply = (fn: (p: DVProgress) => DVProgress) => {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 px-4 py-4">
      ...
      {/* Wordmark header */}
      <div className="flex items-center gap-3">
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-red-600 text-2xl shadow-md">🪷</div>
        <div className="flex-1">
          <div className="text-[10px] font-black uppercase tracking-[0.25em] text-amber-600">Zero to Expert</div>
          <h1 className="text-lg font-black leading-tight text-gray-800">Đọc &amp; Viết <span className="text-sm font-bold text-gray-400">· Learn to read &amp; write Vietnamese</span></h1>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="rounded-full bg-red-100 px-2.5 py-1 text-xs font-black text-red-700">{level.emoji} {level.title}</span>
      </div>
      ...
      <div className="flex rounded-2xl bg-gray-100 p-1 text-sm font-black">
        {([
          ...
          <button key={t.id} onClick={() => { setTab(t.id); setSelectedMonth(null); }}
            className={`flex-1 rounded-xl py-2 transition-all ${tab === t.id ? 'bg-white text-red-700 shadow' : 'text-gray-400'}`}>
            {t.label}
          </button>
        ))}
      </div>
    ...
    <div className="space-y-5">
      ...
      {nextLesson ? (
        <button onClick={() => onLesson(nextLesson)}
          className="flex w-full items-center gap-4 rounded-2xl border-2 border-red-300 bg-gradient-to-r from-red-600 to-red-800 p-4 text-left shadow-lg transition-all hover:shadow-xl">
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-white/20 text-3xl">▶️</div>
          <div className="min-w-0">
            <div className="text-xs font-bold uppercase tracking-wide text-red-200">{doneCount === 0 ? 'Start Learning' : 'Continue Learning'}</div>
            <div className="truncate font-black text-white">{nextLesson.title}</div>
            <div className="text-xs text-red-200">Month {nextLesson.monthIndex + 1} · Week {nextLesson.weekIndex + 1} · ~10 min</div>
          </div>
        </button>
      ) : (
        <div className="rounded-2xl border-2 border-amber-300 bg-amber-50 p-4 text-center">
          <div className="text-3xl">🎓</div>
          <div className="font-black text-amber-800">Programme complete — bạn là chuyên gia!</div>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
        <div className="mb-1 flex justify-between text-xs font-semibold text-gray-500">
          ...
          <span className="font-black text-red-600">{pct}%</span>
        </div>
        <div className="h-2.5 w-full overflow-hidden rounded-full bg-gray-100">
          <div className="h-2.5 rounded-full bg-gradient-to-r from-red-500 to-amber-500 transition-all duration-700" style={{ width: `${pct}%` }} />
        </div>
      </div>
          ...
          {CURRICULUM.map((month) => {
            ...
            return (
              <button key={month.index} onClick={() => unlocked && onMonth(month)} disabled={!unlocked}
                ...
                <div className="text-xs text-gray-500">{month.subtitle}</div>
                <div className="mt-2 text-xs text-gray-400">{monthDone}/{monthLessons.length} lessons · {month.level}</div>
                {monthDone > 0 && (
      ...
      {doneCount === 0 && (
        <button onClick={onPlacement}
          className="flex w-full items-center justify-between rounded-2xl border-2 border-dashed border-gray-300 bg-white p-4 text-left hover:border-red-300">
          <div>
...
// ─── Daily goals ──────────────────────────────────────────────────────────────
function DailyGoals({ progress, hasLessonsLeft }: { progress: DVProgress; hasLessonsLeft: boolean }) {
...
const NAME_KEY = 'dv-name-v1';

function MeTab({ progress, onReset }: { progress: DVProgress; onReset: () => void }) {
  const [ownName, setOwnName] = useState(() => localStorage.getItem(NAME_KEY) ?? '');
  const displayName = ownName.trim() || 'Learner';
  ...
  return (
    <div className="space-y-4">
      {/* Level card */}
      <div className="rounded-2xl bg-gradient-to-br from-amber-400 to-red-600 p-5 text-white shadow-lg">
        <div className="text-4xl">{level.emoji}</div>
        <h2 className="text-xl font-black">{level.title}</h2>
        <p className="text-sm font-semibold opacity-90">{level.en} · {progress.xp} XP total</p>
        {next && (
          <div className="mt-3">
            <div className="mb-1 flex justify-between text-xs font-semibold opacity-90">
              <span>Next: {next.emoji} {next.title} ({next.en})</span>
      ...
      <StepCard>
        ...
        <input
          ...
          className="w-full rounded-xl border-2 border-gray-200 px-3 py-2 text-sm focus:border-red-400 focus:outline-none"
        />
      </StepCard>
      ...
      {/* Certificate */}
      {graduated && (
        <div className="rounded-2xl border-4 border-double border-amber-400 bg-gradient-to-br from-amber-50 to-red-50 p-6 text-center shadow-lg print:border-amber-400">
          <div className="text-4xl">🎓</div>
          <div className="mt-1 text-xs font-black uppercase tracking-[0.3em] text-amber-600">Chứng chỉ hoàn thành · Certificate of Completion</div>
          ...
          <div className="mt-2 text-sm text-gray-500">
            ...
            <br /><strong className="text-gray-700">Đọc &amp; Viết: Zero to Expert</strong> programme
            <br />— all {totalLessons} lessons, from the 29-letter alphabet and six tones to advanced Vietnamese literacy —
          </div>
          <div className="mt-3 text-xs font-bold text-gray-400">{new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })} · {progress.xp} XP earned · Chúc mừng! 🪷</div>
          <button onClick={() => window.print()} className="mt-4 rounded-full bg-red-600 px-5 py-2 text-sm font-black text-white hover:bg-red-700 print:hidden">
            🖨️ Print Certificate
          </button>
        </div>
      )}
