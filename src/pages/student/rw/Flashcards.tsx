...
import { awardOnce, todayISO, type RWProgress } from './engine';
...
// Always-available starter deck (Dolch sight words) so flashcards work from day one
const SIGHT_CARDS: Card[] = [
  { word: 'the', meaning: 'used before a noun: the cat, the sun' },
  { word: 'and', meaning: 'joins two things: bread and milk' },
  { word: 'you', meaning: 'the person I am talking to' },
  { word: 'she', meaning: 'a girl or woman' },
  { word: 'was', meaning: 'past of "is": he was here' },
  { word: 'are', meaning: 'used with you/we/they: you are kind' },
  { word: 'have', meaning: 'to own or hold something' },
  { word: 'they', meaning: 'more than one person or thing' },
  { word: 'this', meaning: 'the thing here, near me' },
  { word: 'from', meaning: 'showing where something starts' },
];
...
export default function Flashcards({ deck, progress, apply, tts, onClose }: {
  deck: Card[]; // learner's vocabulary; sight words fill in when it's small
  progress: RWProgress;
  ...
  const cards = useMemo(() => {
    const pool = deck.length >= 8 ? deck : [...deck, ...SIGHT_CARDS];
    ...
    return (
      <StepCard>
        ...
        <div className="py-6 text-center">
          ...
          <NavButton label="Back to Review →" onClick={onClose} color="bg-violet-600" />
        </div>
      </StepCard>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <button
        ...
        className={`flex min-h-[220px] w-full flex-col items-center justify-center rounded-3xl border-4 p-6 text-center shadow-md transition-all ${
          flipped ? 'border-emerald-300 bg-emerald-50' : 'border-violet-300 bg-white'}`}
      >
        {!flipped ? (
          <>
          ...
          <>
            <div className="text-sm font-black uppercase tracking-wide text-emerald-500">{card.word}</div>
            <div className="mt-2 text-lg font-bold text-gray-700">{card.meaning}</div>
            <button onClick={(e) => { e.stopPropagation(); tts.speak(`${card.word}. ${card.meaning}`); }}
